...
import { singlepost, updatePost } from "@/Redux/postSlice";
...
const UpdateProduct = () => {
  ...
  }, [dispatch, productId]); // ✅ শুধুমাত্র `productId` পরিবর্তন হলে কল হবে
  ...
  useEffect(() => {
    if (productDetails) {
      ...
      setCuttedPrice(productDetails.cuttedPrice);
      setCategory(productDetails.category);
      setStock(productDetails.stock);
      setWarranty(productDetails.warranty);
      setBrand(productDetails.brand?.name);
      setHighlights(productDetails.highlights);
      setSpecs(productDetails.specifications);
      setOldImages(productDetails.images);
      setoldLogo(productDetails.brand?.logo?.url);
    }
  }, [productDetails]); // ✅ `productDetails` পরিবর্তিত হলে state সেট হবে
  ...
  const ProductEditSubmitHandler = (e) => {
    ...
    formData.append("specifications", JSON.stringify(specs)); // ✅ Fixed
    // for (let pair of formData.entries()) {
    //     console.log(pair[0], pair[1]);
    // }
    dispatch(updatePost({ formData, productId })).then((data) => {
  ...
  return (
    <>
      ...
      <form
        onSubmit={ProductEditSubmitHandler}
        ...
        <div className="flex flex-col gap-2 m-2 sm:w-1/2">
          ...
          <div className="flex justify-evenly gap-2 items-center">
            ...
            <span
              onClick={() => addSpecs()}
              className="py-2 px-6 bg-blue-600 text-white rounded hover:shadow-lg cursor-pointer">
              Add
            </span>
          </div>
          ...
          <div className="flex justify-end">
            ...
            />
            {/* <input  type="submit" className="bg-orange-600 uppercase w-1/3 p-3 text-white font-medium rounded shadow hover:shadow-lg cursor-pointer" /> */}
          </div>
        </div>
      </form>
    </>
