...
import { addQuntity, decreaseQuntity } from "@/Redux/CartSlice";
...
const CartItem = ({
  ...
  const increaseQuantity = () => {
    dispatch(addQuntity({ _id }));
  };

  const decreaseQuantity = () => {
    dispatch(decreaseQuntity({ _id }));
  };
  ...
  return (
    <div
      ...
      <Link
        ...
        <div className="w-full sm:w-1/6 h-28 flex-shrink-0">
          <img
            ...
            src={images?.length > 0 ? images[0].url : "/images/default.png"}
            alt={name}
          />
        </div>
        {/* <!-- product image --> */}
      ...
      <div className="flex justify-between pr-4 sm:pr-0 sm:justify-start sm:gap-6">
        ...
        {inCart && (
          <>
            <button className="sm:ml-4 font-medium hover:text-blue-600">
              SAVE FOR LATER
            </button>
            <button className="font-medium hover:text-red-600">REMOVE</button>
          </>
        )}
      </div>
      {/* <!-- save for later --> */}
