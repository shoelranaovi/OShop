...
import { useDispatch, useSelector } from "react-redux";
import { useSnackbar } from "notistack";
import { allproducts } from "@/Redux/postSlice";
...
const Products = () => {
  ...
  const { isLoading, products } = useSelector((state) => state.post);
  ...
  console.log(category, price, ratings, keyword);
  ...
  useEffect(() => {
    ...
    dispatch(
      allproducts({ keyword, category, price, ratings, currentPage })
    ).then((data) => {
      console.log(data);
    });
  }, [
